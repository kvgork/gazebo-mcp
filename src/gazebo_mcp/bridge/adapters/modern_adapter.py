...
class ModernGazeboAdapter(GazeboInterface):
    ...
    async def list_entities(
        ...
        Uses the /world/{world}/scene/info service to get all models.
        ...
        try:
            # Call the Ignition scene/info service
            ...
            if result.returncode != 0:
                self.logger.warning(f"Failed to get scene info for world '{world}'")
                return []

            # Parse the output to extract model names (top-level entities)
            # Look for lines like:   name: "model_name"
            # Filter out nested entities (links, visuals, etc) by looking for top-level models
            models = []
            ...
            for i, line in enumerate(lines):
                # Detect model blocks (top-level, no extra indentation before 'name:')
                if 'model {' in line:
                    in_model = True
            ...
            self.logger.debug(f"Found {len(models)} models in world '{world}': {models}")
            return models

        except subprocess.TimeoutExpired:
            self.logger.error(f"Timeout querying scene info for world '{world}'")
            return []
        except Exception as e:
            self.logger.error(f"Error listing entities in world '{world}': {e}")
            return []
