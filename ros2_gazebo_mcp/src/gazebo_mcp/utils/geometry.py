...
if __name__ == "__main__":
    print("Geometry Utilities Example")
    print("=" * 50)

    # Quaternion operations:
    print("\n1. Quaternion Multiplication:")
    q1 = euler_to_quaternion(0, 0, math.pi/2)  # 90° around Z
    q2 = euler_to_quaternion(math.pi/2, 0, 0)  # 90° around X
    q_result = quaternion_multiply(q1, q2)
    r, p, y = quaternion_to_euler(*q_result)
    print(f"   90° Z + 90° X = roll={math.degrees(r):.1f}°, pitch={math.degrees(p):.1f}°, yaw={math.degrees(y):.1f}°")

    # SLERP:
    print("\n2. Quaternion SLERP:")
    q_start = (0, 0, 0, 1)
    q_end = euler_to_quaternion(0, 0, math.pi/2)
    q_mid = quaternion_slerp(q_start, q_end, 0.5)
    _, _, yaw = quaternion_to_euler(*q_mid)
    print(f"   Halfway between 0° and 90°: {math.degrees(yaw):.1f}°")

    # Vector rotation:
    print("\n3. Vector Rotation:")
    v = (1.0, 0.0, 0.0)  # X-axis
    q_rot = euler_to_quaternion(0, 0, math.pi/2)  # 90° around Z
    v_rotated = rotate_vector(v, q_rot)
    print(f"   (1, 0, 0) rotated 90° around Z: ({v_rotated[0]:.3f}, {v_rotated[1]:.3f}, {v_rotated[2]:.3f})")
    ...
    p2 = (3, 4, 0)
    dist = distance_3d(p1, p2)
    print(f"   Distance from (0,0,0) to (3,4,0): {dist:.1f}")

    # Angular difference:
    print("\n5. Angular Difference:")
    q1 = euler_to_quaternion(0, 0, 0)
    q2 = euler_to_quaternion(0, 0, math.pi/4)
    angle = quaternion_angle_diff(q1, q2)
    print(f"   Angle between 0° and 45°: {math.degrees(angle):.1f}°")
