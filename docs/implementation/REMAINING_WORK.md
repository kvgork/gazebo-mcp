...
> **Live round-trip 2026-07-04 — RAN against headless Harmonic. spawn + step VERIFIED; pose readback BROKEN.** Driver: `gz sim -s empty.sdf` + `parameter_bridge` binary + rclpy driving the real `ModernGazeboAdapter` (probe kept in `.agent-state/20260704-p5-hardening/`).
> 1. **FIXED — `gazebo_bridge.launch.py` topic args were malformed (root cause of a dead bring-up).** parameter_bridge topic syntax is `topic@ROS_type[GZ_type`; the `@ROS_type` before the `[` is REQUIRED. The launch used `/world/{w}/pose/info[ros_gz_interfaces/msg/ParamVec` and `/clock[rosgraph_msgs/msg/Clock` — no `@ROS_type`, no gz type. parameter_bridge treats ONE malformed argv as fatal → prints usage and exits → bridges NOTHING (incl. the 4 services). Corrected to `/world/{w}/pose/info@tf2_msgs/msg/TFMessage[gz.msgs.Pose_V` and `/clock@rosgraph_msgs/msg/Clock[gz.msgs.Clock`. **With the fix: the 4 services bridge, and live `spawn_entity('box')`→True + `step(10)`→success are VERIFIED.** (Service `@`-syntax `svc@ros_gz_interfaces/srv/SpawnEntity` was already correct — the topic args were what aborted the bridge.)
> 2. **NEW BLOCKER (P0-B-real pose readback) — Pose_V→TFMessage bridge emits EMPTY frame names.** Raw `/world/{w}/pose/info` TFMessage transforms arrive with `frame_id=''` AND `child_frame_id=''` in this ros_gz version. `ModernGazeboAdapter` keys `_entity_states[world][child_frame_id]`, so every entity collapses to a single `''` key and `get_entity_state('box')` raises `ModelNotFoundError`. **Fix (redesign):** bridge Pose_V to a name-preserving type (e.g. `ros_gz_interfaces/msg/Pose_V`, parse `.name`) or read the gz-transport `Pose_V` directly, instead of relying on TFMessage `child_frame_id`. Supersedes/expands deferred #1 (was "cross-model link-name collisions").
> 3. **ENV PREREQ — pixi `-e full` ros2 CLI is incomplete.** Only `ros2cli`/`ros2pkg`/`ros2topic` extensions install; **no `ros2 launch`/`run`/`service`/`interface`**. The repo's `ros2 launch gazebo_mcp gazebo_bridge.launch.py` bring-up cannot run here as-is — add the `ros-jazzy-ros2cli`-family packages to the `sim`/`full` pixi feature, or bring up via direct binaries (as the probe does). `rclpy` + the `parameter_bridge` binary ARE present.
> Adapter drivability confirmed: on-demand `spin_until_future_complete`/`spin_once` (no persistent executor needed), so a rclpy driver works without the executor-conflict of deferred #5.
