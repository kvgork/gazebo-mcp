...
>
> **P0-B-real LIVE — ✅ spawn/step/pose round-trip VERIFIED 2026-07-04** (commits `78b2a54` bridge-launch fix, `4e29df8` pose-readback redesign). Against headless Harmonic (`gz sim -s` + `parameter_bridge` + rclpy driving the real `ModernGazeboAdapter`): spawn box@(1,2,0.5) → step(10) → `get_entity_state('box')` = `position [1,2,0.4999999994]`, orientation ~`[0,0,0,1]`. Two live bugs fixed: (a) `gazebo_bridge.launch.py` topic args were malformed (`topic[type` with no `@ROS_type`) → one bad argv aborted the whole bridge incl. services; (b) the Pose_V→TFMessage bridge emits empty frame names, so `get_entity_state` was redesigned to read the name-carrying gz-transport `Pose_V` (`gz topic -e --json-output`) directly. Repro: `.agent-state/20260704-p5-hardening/preal-artifacts/preal_roundtrip.py`. Remaining live-open: pin the bring-up (pixi `-e full` ros2 CLI lacks `launch`/`run`/`service`), `set_physics`/`seed` honesty, detection AUTO→MOCK loud-warn.
>
...
> 1. ~~**`modern_adapter` pose cache keyed only by `child_frame_id`**~~ — **RESOLVED 2026-07-04** (`4e29df8`): pose readback no longer uses a `child_frame_id` cache at all — it reads the name-carrying gz-transport `Pose_V` and matches the top-level model name, so cross-model link-name collisions can't occur. (Original concern: `Pose_V`/`TFMessage` carries every entity incl. links/visuals, so link-name collisions shadowed models.)
...
>
> **P3 #1 (schema fidelity) — ⏳ PARTIALLY RESOLVED 2026-07-04.** `legacy_mount._hint_for` now maps `array`+`items` → `list[item]`, so the unified app re-advertises curated array `items` typing (`items:{type:string|object}`); locked by `tests/integration/test_p3_schema_fidelity.py`. **Intrinsic residual:** FastMCP 1.27.1 derives BOTH the advertised schema AND pre-handler arg validation from one inferred `arg_model`, so a fully-curated `tools/list` schema and lenient handler-produced `OperationResult` errors on a bad-*type* arg are mutually exclusive for a native tool — we favour fidelity + standard JSON-RPC validation. `object` params stay bare `dict` (nested `properties` not synthesized — over-constrains free-form dicts for marginal gain). **Retire of the old `server.py`/`sdk_app.py` is a GATED REFACTOR, not a deletion:** `sdk_app._build_registry()` is the live registry source `legacy_mount` consumes — retire = extract it to a shared module + drop the standalone low-level entrypoint, after an HTTP soak.
>
...
>
> **Grill (3 attacker lenses, all resolved):** F1 non-finite (NaN/inf) wrench/joint now rejected (had bypassed the non-strict magnitude clamp — `nan<=cap` is False); F2 non-finite trajectory positions rejected (finite out-of-range clamp **now DONE 2026-07-05** via the optional `joint_names` trajectory arg + `enforce_trajectory` bridge backstop — see below); F3 `delete_entity` frees the persistent-wrench slot (was leaking → permanent cap denial); F4 registry slot rolled back on adapter failure/False; F5 progress no longer chunks physics (chunking made the mock's non-composable from-rest integration depend on whether a progressToken was attached — observability mutating state); F6 `GAZEBO_RATE_LIMIT_HZ<=0` disables instead of crashing startup. Rate cap is **wrench-topic only** (per-model joint rate-limiting would wrongly throttle legitimate multi-joint control) — documented.
>
> Real ros_gz/Harmonic actuation-bounds behaviour **written but NOT live-verified** (no `-e full`; joint pos backstop at the bridge needs `limits` injected, which only the tool layer does today).
>
> **P5-deferred #1 — per-waypoint trajectory clamping ✅ DONE 2026-07-05** (mock). `actuate_joint_trajectory` gained an optional `joint_names` arg (index-aligned to each waypoint's `positions`); when supplied, each finite waypoint position is validated against that joint's manifest `[lower, upper]` (out-of-range -> `JOINT_LIMIT_EXCEEDED`, unknown -> `UNKNOWN_JOINT`, length mismatch / bad `joint_names` -> `INVALID_TRAJECTORY`; continuous joints unchecked). New `utils/actuation_bounds.enforce_trajectory` is the bridge backstop (`command_joint_trajectory(..., limits=)`): non-finite positions raise in both modes; finite positions clamp (non-strict) or raise (strict). Mirrors the single-joint `enforce_joint`/`command_joint` two-layer design; backward-compatible (no `joint_names` -> shape+timing only). Suite 651 pass / 11 skip / 0 fail.
