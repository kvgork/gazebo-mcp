...
class TestPauseSimulation:
    ...
    def test_pause_simulation_real_mode_success(self):
        ...
        with patch.object(simulation_tools, 'use_real_gazebo', return_value=True):
            with patch.object(simulation_tools, 'get_bridge', return_value=mock_bridge):
                result = simulation_tools.pause_simulation(timeout=5.0)

        assert result.success is True
        assert result.data["paused"] is True
        assert "timestamp" in result.data
        mock_bridge.pause_physics.assert_called_once()
...
class TestUnpauseSimulation:
    ...
    def test_unpause_simulation_real_mode_success(self):
        ...
        assert result.success is True
        assert result.data["paused"] is False
        mock_bridge.unpause_physics.assert_called_once()
...
class TestResetSimulation:
    ...
    def test_reset_simulation_real_mode_success(self):
        ...
        assert result.data["reset"] is True
        assert result.data["simulation_time"] == 0.0
        mock_bridge.reset_simulation.assert_called_once()
...
class TestSetSimulationSpeed:
    ...
    def test_set_simulation_speed_gazebo_error(self):
        ...
        with patch.object(simulation_tools, 'use_real_gazebo', side_effect=RuntimeError("Error")):
            result = simulation_tools.set_simulation_speed(1.0)
...
class TestGetSimulationTime:
    ...
    def test_get_simulation_time_real_mode(self):
        """Test getting time in real mode reads from bridge.get_simulation_stats()."""
        mock_bridge = Mock()
        mock_bridge.get_simulation_stats.return_value = {
        ...
        assert result.success is True
        assert result.data["simulation_time"] == 42.5
        assert result.data["real_time"] == 43.1
        assert result.data["iterations"] == 42500
        assert "paused" in result.data
        mock_bridge.get_simulation_stats.assert_called_once()
...
class TestListWorlds:
    """Tests for list_worlds()."""

    def test_list_worlds_mock_mode(self):
        """Test list_worlds returns default world when Gazebo not running."""
        with patch.object(simulation_tools, 'use_real_gazebo', return_value=False):
            result = simulation_tools.list_worlds()

        assert result.success is True
        assert "worlds" in result.data
        assert "count" in result.data
        assert result.data["count"] == 1
        assert "default" in result.data["worlds"]
        assert "note" in result.data

    def test_list_worlds_real_mode_uses_discover(self):
        """Test list_worlds calls _discover_worlds in real mode."""
        with patch.object(simulation_tools, 'use_real_gazebo', return_value=True):
            with patch.object(simulation_tools, '_discover_worlds', return_value=["my_world"]) as mock_discover:
                result = simulation_tools.list_worlds()

        assert result.success is True
        assert result.data["worlds"] == ["my_world"]
        assert result.data["count"] == 1
        mock_discover.assert_called_once()

    def test_discover_worlds_fallback_on_no_cli(self):
        """Test _discover_worlds returns ['default'] when gz/ign CLI not available."""
        import subprocess
        with patch('subprocess.run', side_effect=FileNotFoundError("gz not found")):
            worlds = simulation_tools._discover_worlds()

        assert worlds == ["default"]

    def test_discover_worlds_parses_gz_output(self):
        """Test _discover_worlds parses world names from gz service list output."""
        import subprocess
        mock_output = "/world/my_world/control\n/world/my_world/create\n/world/other/control\n"
        mock_result = Mock()
        ...
        assert "my_world" in worlds
        assert "other" in worlds

    def test_list_worlds_handles_exception(self):
        """Test list_worlds returns error on unexpected exception."""
        with patch.object(simulation_tools, 'use_real_gazebo', side_effect=RuntimeError("unexpected")):
            result = simulation_tools.list_worlds()

        assert result.success is False
        assert result.error_code == "LIST_WORLDS_ERROR"
