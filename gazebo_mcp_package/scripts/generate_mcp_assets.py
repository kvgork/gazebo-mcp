...
CLAUDE_ROOT = os.environ.get("CLAUDE_ROOT")
if CLAUDE_ROOT:
    CLAUDE_ROOT = Path(CLAUDE_ROOT)
else:
    # Try to find claude in common relative locations
    possible_paths = [
        PROJECT_ROOT.parent / "claude",
        Path.home() / "workspaces" / "claude",
        Path.home() / "projects" / "claude",
    ]
...
if not CLAUDE_ROOT:
    print("Error: Could not find claude project directory")
    print("  Set CLAUDE_ROOT environment variable or place claude as sibling directory")
    print("  Example: export CLAUDE_ROOT=/path/to/claude")
    sys.exit(1)
...
try:
    ...
    print(f"  {e}")
    print(f"\nClaude directory found at: {CLAUDE_ROOT}")
    print("  Make sure it contains the required skills modules")
    sys.exit(1)
