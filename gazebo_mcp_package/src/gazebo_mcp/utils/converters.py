...
if __name__ == "__main__":
    # Quaternion ↔ Euler conversion example:
    print("Quaternion ↔ Euler Conversion:")
    print("-" * 40)

    # Convert Euler to quaternion:
    roll, pitch, yaw = 0.0, 0.0, math.pi / 2  # 90° rotation around Z
    x, y, z, w = euler_to_quaternion(roll, pitch, yaw)
    print(f"Euler (0, 0, π/2) → Quaternion ({x:.3f}, {y:.3f}, {z:.3f}, {w:.3f})")

    # Convert back to Euler:
    roll2, pitch2, yaw2 = quaternion_to_euler(x, y, z, w)
    print(f"Quaternion → Euler ({roll2:.3f}, {pitch2:.3f}, {yaw2:.3f})")

    print("\n" + "=" * 40 + "\n")

    # Dictionary conversion example (without ROS2):
    print("Dictionary Conversion (no ROS2):")
    print("-" * 40)
    ...
    # Validate quaternion from dict:
    orient = pose_dict["orientation"]
    x, y, z, w = validate_quaternion(
        orient["x"], orient["y"], orient["z"], orient["w"]
    )
    print(f"Validated quaternion: ({x:.3f}, {y:.3f}, {z:.3f}, {w:.3f})")
